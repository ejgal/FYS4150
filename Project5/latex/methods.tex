...
that it is nonlinear. In order to obtain linear wave solutions we need to
...
Inserting  \cref{eq:Beta_approx} and \cref{eq:VeloctiyPertubation} in the
...
one dimension, first with periodic boundary conditions and then with closed
boundaries.
...
\subsubsection{Closed boundaries}
In the ocean the boundaries would be better represented using closed boundary
conditions. We will suppose a wavelike solution of \cref{eq:barotropicRossby}
...
can split the equation into two equations, one for the sine and one cosine
...
In this section we explain the implementation of the numerical model and explore
stability .
Further details can be found in the python code in the github repository of
this project
\footnote{\url{https://github.com/Ovewh/Computilus/tree/master/Project5}}.
...
$-0.0063$ for the periodic case and $c = -0.012665$ for the closed boundary case.
...
$(\Delta x)^2$ and
$(\Delta t)^2$, while the forward difference term has a truncation error
of order $\Delta t$
...
For the CTCS scheme, $\zeta_{j}^{1}$ is a function of $\zeta_{j}^{-1}$ which we don't know.
...
of poissons equation, one closed and one periodic.
...
For the CTCS scheme with a sinusoidal inital state with k = $4 \pi$ and
$\Delta x = \frac{1}{40}$
