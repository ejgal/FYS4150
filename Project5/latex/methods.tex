...
\end{equation}
To linearise the Coriolis parameter $f=2\Omega \sin(\theta)$, $\Omega$ is the
rotation rate of the earth ($\Omega = 2\pi/ \mathrm{day}$). 
...
\begin{figure}[htp]
  ...
  compared to analytical solutions of poissons equation. Convergence parameter set to (from top to bottom) 10$^{-6}$, 10$^{-8}$, 10$^{-10}$ }
  \label{fig:periodic_sine}
\end{figure}

Since we are using close to 40 grid points for our simulations we ended up
iterating until the difference between the iterations was less than 10$^{-8}$.
