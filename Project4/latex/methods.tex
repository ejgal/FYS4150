...
distribution contains the partition function $Z$, which goes like $2^{O}$ and
for larger lattices it becomes impossible compute. This is where the Metropolis
algorithm comes to our rescue. The idea behind the Metropolis algorithm is to
randomly sample a probability distribution and then we update our current
probability distribution for step n and state i $P_{i}^(n) by the two following
rules. 
\
