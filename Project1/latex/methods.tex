...
here we will just outline the general idea. \par The Thomas algorithm consist of
two steps. The first steps is to decompose the matrix $\bvec{A}$ into an upper
triangular matrix $\bvec{U}$ and a lower triangular matrix $\bvec{L}$
\cite{tridia}. The first step is step is done with a forward iteration where we
decompose the matrix and solve for $L  