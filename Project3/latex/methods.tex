...
domain. We will use a random number generator (RNG) to select the values where
we evaluete the function we are integrating. As the sample size is increased
the more certain we become about our estimation, and when the sample size
...
\hat{I} = f(X)$, where X is a random variable. It has expectation value
$\braket{\hat{I}} = \int_{0}^{1} f(x)p(x)dx$, where $p(x)$ is the probability
density function of the random variable $X$. If $p(x)$ is the uniform distribution $p(x) = 1$ for $x \in
...
standard deviation by choosing a better estimator.
One way of doing this is with importance sampling. Instead of sampling X from
the uniform distribution we will sample from a distribution with probability
density function proportional to the integrand. We would then evaluate the integrand
more often where it is large than where it is small. To account for this bias we
also change our estimator to \cref{eq:new_estimator}, which also have expectation
value $\int_{-\infty}^{\infty} f(\*x) d\*x$.
